...
Maximizing the \text{log} likelihood in the training step 

**a probabilistic model**

The dataset is made of $m$ training examples $(x(i), y(i))_{i\in[m]}$, where 

$$\mathcal{L}(\theta \mid x) = \text{log} L(\theta)
                             = -m\text{log} (\sigma \sqrt{2\pi}) - \frac{1}{2\sigma^{2}}\sum_{i=1}^{m}(y(i) - \theta^T x(i))^2$$
...
In the context of linear regression, the \text{log}-likelihood is often associated with the assumption of normally distributed errors. The typical formulation assumes that the response variable follows a normal distribution with a mean determined by the linear regression model. Here's how you can express the \text{log}-likelihood for a simple linear regression model:
...
The \text{log}-likelihood for the entire dataset $(\{y_1, y_2, \ldots, y_n\})$ is the sum of the \text{log}-likelihood contributions from each observation:

$[ \mathcal{L}(\beta_0, \beta_1, \sigma^2 \mid x_i) = -\frac{n}{2}\text{log}(2\pi\sigma^2) - \frac{1}{2\sigma^2}\sum_{i=1}^{n}(y_i - \mu_i)^2 ]$

Here:
- $(\beta_0)$ and $(\beta_1)$ are the coefficients of the linear regression model.
- $(n)$ is the number of observations.

The goal in linear regression is often to maximize this \text{log}-likelihood function, which is equivalent to minimizing the sum of squared residuals (ordinary least squares approach).

Note that in practice, maximizing the \text{log}-likelihood is often done under the assumption that the errors ($(y_i - \mu_i)$) are normally distributed, which allows for the use of maximum likelihood estimation (MLE). This assumption is a key aspect of classical linear regression.
...
There is no closed form formula for $argmax \mathcal(\theta)$ so that we need now to use iterative algorithms. The gradient of the \text{log} likelihood is:
...
$$ \text{loss}(i) = -[\text{y}(i)\text{\text{log}} \text{z}(i) + (1 - \text{y}(i)) \text{\text{log}}(1 - \text{z}(i))]$$
$$ \text{BCELoss(z,y) = \frac{1}{m} \sum{i=1}{m} \text{loss}(i)}$$
